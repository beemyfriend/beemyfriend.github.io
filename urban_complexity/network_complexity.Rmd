...
<h1><a href='https://geoffboeing.com/2018/07/comparing-city-street-orientations/'>Geoff Boeing did something cool</a></h1>
...
<h2>Geoff did something cool </h2>
...
and this will provide us with a matrix of the maximum and minmum values for the regions longitudal (x) and latitudal (y) borders
...
Once we define the region we care about, we can pull data from **OpenStreetMap** by building an *Overpass Query*. Since we're only interested in building a street network, we will only pull the *highway features*. Once tell OSM the features we want, we can extract the *linestring* geometry as a **Simple Feature**.
...
What we get is a cool an *igraph* style graph with the logtitude (x) and latitude (y) coordinates of the intersections (nodes) stored as graph attributes
...
- I binned at every 10 $^\circ$ starting at 90 $^\circ$ and ending at -90 $^\circ$. I'm not sure how Geoff binned his roads. 
